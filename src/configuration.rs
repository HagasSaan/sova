...
impl Configuration {
    pub fn load(path: &str) -> Configuration {
        ...
        serde_yaml::from_str(&config_str).expect("Error on loading configuration from content")
    }

    pub fn save(&self, path: &str) {
        let config_str = serde_yaml::to_string(&self).expect("Error on dumping config to string");
...
pub fn load_configuration() -> Configuration {
    ...
    if !Path::new(config_path).exists() {
        fs::create_dir_all("/etc/sova").expect("Failed to create /etc/sova directory");
        build_default_configuration().save(config_path);
    }
